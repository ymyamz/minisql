...
AbstractPlanNodeRef Planner::PlanSelect(std::shared_ptr<SelectStatement> statement) {
  ...
  // cout << "column_in_condition_" << endl;
  // for(auto x : statement->column_in_condition_)
  //   cout << x << ' ';
  // cout << endl;
  for (auto index : indexes) {
    // cout << "index:" << index->GetIndexKeySchema()->GetColumns().size() << endl;
    if (index->GetIndexKeySchema()->GetColumns().size() == 1) {//单列索引，我们才能用
      // cout << "Ind:" << index->GetIndexKeySchema()->GetColumn(0)->GetTableInd() << endl;
  ...
  return make_shared<IndexScanPlanNode>(out_schema, statement->table_name_, available_index,
                                        ...
                                        statement->where_);//available index里已经列出了可用的index
}
