...
TEST_F(ExecutorTest, SimpleSeqScanTest) {
  ...
  puts("1");
  cout << GetExecutorContext()->GetCatalog()->GetTable("table-1", table_info);
  puts("1.5");
  const Schema *schema = table_info->GetSchema();
  assert(schema != nullptr);
  auto vector = schema->GetColumns();
...
TEST_F(ExecutorTest, SimpleDeleteTest) {
  ...
  vector<Field> Fields;
  Fields.clear();
  Fields.emplace_back(*result_set[0].GetField(0));
  const Row index_key = Row(Fields);
